...
getChar(istream& istr)
{
   ...
   switch (ch) {
      ...
      case ESC_KEY: {
	    ...
	    if(key>=char(ARROW_KEY_BEGIN) && char(key<=ARROW_KEY_END))  return returnCh(int(key) + ARROW_KEY_FLAG);
	    //else if(key>=char(END_KEY) && key<=char(HOME_KEY))	return returnCh(int(key) + MOD_KEY_FLAG);
	    else if(key>=char(MOD_KEY_BEGIN) && key<=char(MOD_KEY_END)){  
		char dummy = mygetc(istr);
		if(dummy==MOD_KEY_DUMMY) return returnCh(int(key) + MOD_KEY_FLAG);
