...
public:
   ...
   void reset(size_t b = 0) {
      ...
      while(_activeBlock -> _nextBlock){
          MemBlock<T>* delBlock = _activeBlock;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(currentList != NULL){
         if(currentList -> _arrSize == n) return currentList;
         if(! currentList -> _nextList) break;
         currentList = currentList -> _nextList;      
      }
      // not found
      currentList -> _nextList = new MemRecycleList<T>(n);
      return currentList -> _nextList;
   }
