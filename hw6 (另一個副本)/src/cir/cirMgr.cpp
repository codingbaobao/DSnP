...
{
	IdList printAIG;
	canReachFromPO(printAIG);
	size_t si=pin.size(),so=pout.size(),sa=printAIG.size(),gateV;
	outfile<<"aag "<<max<<" "<<si<<" 0 "<<so<<" "<<sa<<endl; //header
	for(size_t i=0;i<si;outfile<<(pin[i++]->varno)*2<<endl); //PI
	for(size_t i=0;i<so;++i) //PO
	{
		gateV=pout[i]->fanin[0];
		outfile<<(CirGate::getGatePtr(gateV)->varno)*2+CirGate::isInv(gateV)<<endl;
	}
	for(size_t i=0;i<sa;++i) //AIG
	{
		CirGate *AIGgate=cirAnd.find(printAIG[i])->second;
		outfile<<(AIGgate->varno)*2;
		gateV=AIGgate->fanin[0];
		outfile<<" "<<(CirGate::getGatePtr(gateV)->varno)*2+CirGate::isInv(gateV);
		gateV=AIGgate->fanin[1];
		outfile<<" "<<(CirGate::getGatePtr(gateV)->varno)*2+CirGate::isInv(gateV)<<endl;
	}
