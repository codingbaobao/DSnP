   ...
   ~CirMgr()
	{
		...
		//for(GateList::iterator i=pin.begin(),e=pin.end();i!=e;delete CirGate::getGatePtr(i++));
		//for(GateList::iterator i=pout.begin(),e=pout.end();i!=e;delete CirGate::getGatePtr(i++));
...
private:
	...
	CirMap cirAnd; //and gate+const gate
	CirMap cirPO;
	CirMap cirPI;
	CirGate* constGate;
	...
	void canReachFromPO(IdList &gate) const
	{
		CirGate::setglobal();
